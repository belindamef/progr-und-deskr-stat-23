...
H_post_bdi = as.data.frame(table(post_bdi))    # absolute Haeufigkeitsverteilung (dataframe)
names(H_post_bdi) = c("a", "h")         # Spaltenbenennung
H_post_bdi$r = H_post_bdi$h/n_post_bdi
...
H_diff_bdi = as.data.frame(table(bdi_diff))    # absolute Haeufigkeitsverteilung (dataframe)
names(H_diff_bdi) = c("a", "h")         # Spaltenbenennung
H_diff_bdi$r = H_diff_bdi$h/n_bdi_diff
...
H_bdi_diff_f2f = as.data.frame(table(bdi_diff_f2f))    # absolute Haeufigkeitsverteilung (dataframe)
H_bdi_diff_ONL = as.data.frame(table(bdi_diff_ONL))    # absolute Haeufigkeitsverteilung (dataframe)
...
# Achtung: das Ergebnis von seq(b_0_f2f, b_k_f2f, by = h) ist 
# (-12,  -9,  -6,  -3,   0), was nicht alle Werte abdeckt. Deshalbt fügen wir 
# einen break (i.e. die 3) hinzu
b_h3_f2f = c(b_h3_f2f, 3)
b_h3_ONL    = seq(b_0_ONL, b_k_ONL, by = h)       # Klassen [b_{j-1}, b_j[
# Achtung: wie bei der ONL Gruppe, deckt auch hier das Ergebnis von seq() 
...
b_excel_f2f = seq(b_0_f2f, b_k_f2f, len = k_f2f)
b_excel_ONL    = seq(b_0_ONL, b_k_ONL, len = k_ONL)

# c) Sturges Klassenzahl
k_f2f         = ceiling(log2(n_f2f) + 1)   # Anzahl der Klassen
k_ONL            = ceiling(log2(n_ONL) + 1)      # Anzahl der Klassen
b_sturges_f2f = seq(b_0_f2f, b_k_f2f, len = k_f2f)
b_sturges_ONL   = seq(b_0_ONL, b_k_ONL, len = k_ONL)
...
b_scott_f2f = seq(b_0_f2f, b_k_f2f, len = k_f2f)
b_scott_ONL    = seq(b_0_ONL, b_k_ONL, len = k_ONL)
...
for (breaks_method_name in names(breaks_list_f2f)) {
  ...
  hist(                                                    # Histogramm
    ...
    main   = paste("Post-Pre-BDI Differenz (f2f),", 
                   breaks_method_name)
    )  
...
for (breaks_method_name in names(breaks_list_ONL)) {
  ...
  hist(                                                    # Histogramm
    ...
    ylab   = "Häufigkeit",                                 # y-Achsenbezeichnung
    xlab   = "",                                           # x-Achsenbezeichnung
    main   = paste("Post-Pre-BDI Differenz (ONL),", 
                   breaks_method_name)
  )  
